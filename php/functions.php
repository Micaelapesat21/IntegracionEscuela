...
function setReaderForEmployeePairing($employeeId)
{
    ...
    echo "<script>console.log('estoy seteando');</script>"; 
...
function assignRfidCard($employee) {
   
    $shouldCheck = true;
    $employeeId = $employee.id;
     
     $employee.$waiting = true;
     $statement -> fetch("./set_reader_for_pairing.php?employee_id=" + $employeeId);
    
     return checkIfEmployeeHasJustAssignedRfid($employee);
 },

 function checkIfEmployeeHasJustAssignedRfid($employee) {
     $r =  fetch("./get_employee_rfid_serial_by_id.php?employee_id=" + $employee.$id);
     $serial = $r.json();
     ...
     if ($serial) {
          ...
          return setReaderForReading();
         
     } else {
        // setTimeout(() => {
          //  checkIfEmployeeHasJustAssignedRfid($employee);
